...
def last_block():
    ...
    if not last: return jsonify({'message': 'Could not retrieve last block'}), 400
    return jsonify(last), 200


@app.route('/mine', methods=['POST'])
def mine():
    # Pull data out of request body and validate proof
    ...
    response = {'message': ''}
    if (not proof and proof is not 0) or not data['id']:
        response['message'] = 'You must provide valid proof and an id'
